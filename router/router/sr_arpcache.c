...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
    struct sr_arpreq *walker=sr->cache.requests;
    while (walker) {
        struct sr_arpreq *next  =  walker->next;
...
void handle_arpreq (struct sr_instance *sr, struct sr_arpreq * req) {
    ...
    if (difftime(now, req->sent) >= 1.0){
        ...
        /*times >= 5, destination host unreachable message(type 3, code 1) */
        if ((req->times_sent) >= 5) {
            while (packet!=NULL) {
                ...
                /*send icmp message*/
                icmp_handler(sr, eth_hdr, ip_hdr, interface, 3, 1);
                packet = packet->next;
            }
            ...
            struct sr_if *itface = sr_get_interface(sr, packet->iface);
            
            /* ethernet header */
            sr_ethernet_hdr_t *new_eth_hdr = (sr_ethernet_hdr_t *) new_packet;
            memset(new_eth_hdr->ether_dhost, 0xff, sizeof(uint8_t)*ETHER_ADDR_LEN);
            ...
            memset(new_arp_hdr->ar_tha, 0Xff, sizeof(unsigned char)*ETHER_ADDR_LEN);
