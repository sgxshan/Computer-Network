        ...
        char* interface/* lent */)
{
    ...
    /*it is an IP packet(enable nat or not)*/
    if (ntohs(eth_hdr->ether_type) == ethertype_ip){
        if(sr->en_nat)
            nat_handler(sr, packet, len, interface);
        else
...
void nat_handler(struct sr_instance *sr, uint8_t *packet, unsigned int len, char *interface){
    sr_ip_hdr_t * ip_hdr = (sr_ip_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t));
    
    /*handle icmp packet*/
    if(ip_hdr->ip_p == ip_protocol_icmp)
        nat_icmp_handler(sr, packet, len, interface);
    
    /*handle tcp packet*/
    else
        nat_tcp_handler(sr, packet, len, interface);
}


void nat_icmp_handler(struct sr_instance* sr, uint8_t * packet, unsigned int len, char* interface){
    sr_ip_hdr_t *ip_hdr = (sr_ip_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t));
    sr_icmp_hdr_t *icmp_hdr = (sr_icmp_hdr_t *)(packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
    struct sr_if *itface = get_ip_interface(sr, ip_hdr->ip_dst);
    if(strncmp(interface, "eth1", 4) == 0){
        ...
        if (!mapping)   mapping = sr_nat_insert_mapping(sr, &(sr->nat), ip_hdr->ip_src, icmp_hdr->icmp_id, nat_mapping_icmp);
...
void nat_tcp_handler(struct sr_instance* sr, uint8_t * packet, unsigned int len, char* interface){
    sr_ethernet_hdr_t* eth_hdr = (sr_ethernet_hdr_t *) packet;
    ...
    if(strncmp(interface, "eth1", 4) == 0){
        ...
        struct sr_nat_mapping *mapping = sr_nat_lookup_internal(&(sr->nat), ip_hdr->ip_src, ntohs(tcp_hdr->src_port), nat_mapping_tcp);
        if (!mapping)   mapping = sr_nat_insert_mapping(sr, &(sr->nat), ip_hdr->ip_src, ntohs(tcp_hdr->src_port), nat_mapping_tcp);
        
        pthread_mutex_lock(&(sr->nat.lock));
        struct sr_nat_connection *cons = sr_lookup_conn(mapping, ip_hdr->ip_dst);
        if(cons==NULL)
            cons = sr_nat_insert_connection(mapping, ip_hdr->ip_dst);
        
        switch(cons->conns_state){
            case closed:
                if(tcp_hdr->syn)
                    cons->conns_state = outbd_syn;
                break;
            case inbd_syn:
                cons->conns_state = establish;
                break;
            case establish:
                if(tcp_hdr->fin)
                    cons->conns_state = outbd_fin;
                break;
            case inbd_fin:
                cons->conns_state = closed;
                break;
            default: break;
        }
        ...
        tcp_hdr->tcp_sum = 0;
        tcp_hdr->tcp_sum = cal_tcpsum(packet, len);
        ip_hdr->ip_src = sr_get_interface(sr, "eth2")->ip;
        ip_hdr->ip_sum = 0;
        ip_hdr->ip_sum = cksum(ip_hdr, sizeof(sr_ip_hdr_t));
        route_packet(sr, packet, len, interface);
    ...
    if(strncmp(interface, "eth2", 4) == 0){
        ...
        {
            if(tcp_hdr->syn){
                struct inbd_syn_t *inbd_pkt = (sr->nat).inbd_syn_packet;
                inbd_pkt = malloc(sizeof(struct inbd_syn_t));
                inbd_pkt->packet = (uint8_t *)malloc(len);
                memcpy(inbd_pkt->packet, packet, len);
                inbd_pkt->port = tcp_hdr->dst_port;
                inbd_pkt->last_updated = time(NULL);
                inbd_pkt->next =(sr->nat).inbd_syn_packet;
                (sr->nat).inbd_syn_packet = inbd_pkt;
            }
            return;
        }
        
        pthread_mutex_lock(&(sr->nat.lock));
        struct sr_nat_connection *cons = sr_lookup_conn(mapping, ip_hdr->ip_src);
        if(cons!=NULL){
            switch(cons->conns_state){
                case outbd_syn:
                    if(tcp_hdr->syn&&tcp_hdr->ack)
                        cons->conns_state = inbd_syn;
                    break;
                    
                case outbd_fin:
                    if(tcp_hdr->fin&&tcp_hdr->ack)
                        cons->conns_state = inbd_fin;
                    break;
                default: break;
            }
        }
...
uint16_t cal_tcpsum(void *packet, unsigned int len)
{
    sr_ip_hdr_t *ip_hdr = (sr_ip_hdr_t *)(packet + sizeof(sr_ethernet_hdr_t));
    sr_tcp_hdr_t *tcp_hdr = (sr_tcp_hdr_t *)(packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
    int tcp_length = len - sizeof(sr_ethernet_hdr_t) - sizeof(sr_ip_hdr_t);
    int pseudo_length = sizeof(sr_pseudo_hdr_t) + tcp_length;
    sr_pseudo_hdr_t *pseudo_hdr = malloc(sizeof(sr_pseudo_hdr_t) + tcp_length);
    pseudo_hdr->ip_src = ip_hdr->ip_src;
    pseudo_hdr->ip_dst = ip_hdr->ip_dst;
    pseudo_hdr->zero = 0;
    pseudo_hdr->ip_p = ip_protocol_tcp;
    pseudo_hdr->tcp_len = htons(tcp_length);
    memcpy((uint8_t *)pseudo_hdr + sizeof(sr_pseudo_hdr_t), (uint8_t *)tcp_hdr, tcp_length);
    uint16_t check_sum = cksum(pseudo_hdr, pseudo_length);
    return check_sum;
}
...
void icmp_handler(struct sr_instance* sr, sr_ethernet_hdr_t* eth_hdr, sr_ip_hdr_t* ip_hdr, char* interface, int type, int code){
    ...
    if(lpm){
        ...
        {
            /* change ip header */
            memcpy(eth_hdr->ether_shost, itface->addr, ETHER_ADDR_LEN);
...
struct sr_rt * get_lpm (struct sr_instance* sr, uint32_t ip_dst) {
    ...
    while (walker) {
        ...
        if ((ip_dst & walker->mask.s_addr) == (walker->dest.s_addr & walker->mask.s_addr)) {
            if (((ip_dst & walker->mask.s_addr) > max_len)||longest_prefix == NULL){
